...
//!
//! 行由本控件绘制（非子控件）；滚动复用 `Base.scrollable/scroll_y/content_h`（分发器
//! 的 scroll_at 处理滚轮）；点击选中经指针转发到 `on_event`，选择变化经 name 上报。
...
impl ListView {
    pub fn new() -> Self {
        let mut base = Base::new(WidgetRole::ListView);
        base.scrollable = true;
        Self { base, items: Vec::new(), row_h: 28.0 }
    }
    /// 设置列表项。
...
impl Widget for ListView {
    ...
    fn arrange(&mut self, content: Rect, _cv: &dyn Canvas) {
        ...
        self.base.content_h = self.items.len() as f32 * self.row_h;
        let max = (self.base.content_h - content.size.height).max(0.0);
        self.base.scroll_y = self.base.scroll_y.clamp(0.0, max);
    }
    fn paint_content(&self, cv: &mut dyn Canvas, style: &StyleSpec) {
        ...
        for (i, item) in self.items.iter().enumerate() {
            ...
            if self.base.selected && self.base.selected_index == i {
        ...
        if self.base.content_h > content.size.height {
            let track_w = 4.0;
            let vh = content.size.height;
            let thumb_h = (vh * vh / self.base.content_h).max(20.0);
            let max_scroll = self.base.content_h - vh;
    ...
    fn on_event(&mut self, ev: &Event) -> EventFlow {
        if let Event::MouseDown { pos, button: MouseButton::Left } = ev {
            let content = layout::content_rect(&self.base);
            let rel = pos.y - content.top() + self.base.scroll_y;
            if rel >= 0.0 {
                let row = (rel / self.row_h) as usize;
                if row < self.items.len() {
                    self.base.selected_index = row;
                    self.base.selected = true;
                }
            }
