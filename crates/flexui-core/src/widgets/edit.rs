...
impl Edit {
    ...
    fn line_layout(&self, cv: &dyn Canvas, li: usize) -> Option<Ref<'_, TextLayout>> {
        ...
        Some(Ref::map(line.layout.borrow(), |slot| {
            slot.as_ref().unwrap()
        }))
    }
    ...
    fn est_col(&self, local_x: f32) -> usize {
        ((local_x / (self.base.font.size * 0.6).max(1.0))
            .round()
            .max(0.0)) as usize
    }
    ...
    fn hit_index(&self, pos: Point) -> usize {
        ...
        let local_x = pos.x - content.left() + if self.config.multiline { 0.0 } else { offset.x };
    ...
    fn paint_multiline(&self, cv: &mut dyn Canvas, style: &StyleSpec) {
        ...
        let static_content = !self.base.focused && sel.is_none() && self.state.marked.is_empty();
        if static_content
            && self
                .paint_text_band(cv, content, line_h, offset_y, color)
                .is_some()
        {
    ...
    fn update_single_line_scroll(&self, caret: f32, total: f32, content: Rect) {
        ...
        state.set_metrics(Size::new(total + 2.0, content.size.height), content.size);
        // 让光标（含 2px 留白）保持在视口内。
