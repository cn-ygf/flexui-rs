...
pub fn apply_localizations(root: &mut Node, localizer: &Localizer) {
    visit_all_mut(root.as_mut(), &mut |widget| {
        ...
        for binding in bindings {
            match binding {
                LocalizationBinding::Text(resource) => {
                    widget.set_text_value(localizer.text(resource))
                }
                ...
                LocalizationBinding::Items(resources) => {
                    let items = resources
                        .into_iter()
                        .map(|resource| localizer.text(resource))
                        .collect();
...
mod tests {
    ...
    fn refreshes_bound_text_and_placeholder_without_replacing_edit_value() {
        ...
        localizer
            .load_json_str(r#"{"locale":"en","strings":{"title":"Settings","hint":"Phone"}}"#)
            .unwrap();
        localizer
            .load_json_str(r#"{"locale":"zh-Hans","strings":{"title":"设置","hint":"手机号"}}"#)
            .unwrap();
        let mut label = Label::new("");
        label
            .base_mut()
            .localizations
            .push(LocalizationBinding::Text("title".into()));
        let mut edit = Edit::new().text("138");
        edit.base_mut()
            .localizations
            .push(LocalizationBinding::Placeholder("hint".into()));
